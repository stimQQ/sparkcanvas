...
RUNNINGHUB_WEBAPPS_FILE = os.path.join(DATA_DIR, "runninghub_webapps.json")
...
SUPPORTED_PROVIDER_PROTOCOLS = {"openai", "apimart", "apimodels", "gemini", "volcengine", "runninghub"}
RUNNINGHUB_DEFAULT_BASE_URL = "https://www.runninghub.cn"
RUNNINGHUB_DEFAULT_IMAGE_MODELS = [
    "seedream-v5-lite/text-to-image",
    "seedream-v5-lite/image-to-image",
]
APIMODELS_DEFAULT_BASE_URL = "https://apimodels.app/api/v1"
# 钉在最前面的"推荐顺序",每次启动会强制提到列表顶部(无论历史保存顺序)。
APIMODELS_PRIORITY_MODELS = [
    "gpt-image-2-lite",
    "doubao-seedream-5-0-260128",
    "nanobanana-2-beta/gemini-3.1-flash-image-preview",
]
APIMODELS_DEFAULT_IMAGE_MODELS = [
    # 优先展示
    "gpt-image-2-lite",
    "doubao-seedream-5-0-260128",
    "nanobanana-2-beta/gemini-3.1-flash-image-preview",
...
def default_api_providers():
    # 默认 ApiModels 排在最前并标记为 primary,新装即可直接用;ModelScope / RunningHub 保留为强制默认平台
    return [
        {
            "id": "apimodels",
            "name": "ApiModels",
            "base_url": APIMODELS_DEFAULT_BASE_URL,
            "protocol": "apimodels",
            "image_generation_endpoint": "",
            "image_edit_endpoint": "",
            "enabled": True,
            "primary": True,
            "image_models": APIMODELS_DEFAULT_IMAGE_MODELS,
            "chat_models": [],
            "video_models": [],
            "ms_loras": [],
...
def merge_default_api_providers(providers):
    ...
    am_default = next((d for d in default_api_providers() if d["id"] == "apimodels"), None)
    if am_default:
        current = next((item for item in merged if item.get("id") == "apimodels"), None)
        if not current:
            merged.append(am_default)
        else:
            if not current.get("base_url"):
                current["base_url"] = am_default["base_url"]
            if not current.get("protocol") or current.get("protocol") == "openai":
                current["protocol"] = "apimodels"
            # 把旧版本默认下来的小写 "apimodels" 升级为品牌名 "ApiModels"
            if str(current.get("name") or "").strip().lower() == "apimodels":
                current["name"] = am_default["name"]
            # 没有任何 provider 标 primary 时,把 apimodels 设为默认首选
            if not any(p.get("primary") for p in merged):
                current["primary"] = True
            # 清掉历史上我们错猜的或已下线的 ID(SparkPix 已统一异步,不再单独成型)
            LEGACY_BAD_IDS = {
                "grok-image", "sparkpix", "gemini-image", "doubao-seedream", "kling-omni-image",
                "sparkpix-image", "sparkpix-image-edit",
            }
            cleaned = [m for m in (current.get("image_models") or []) if m not in LEGACY_BAD_IDS]
            # 强制把 priority 模型提到列表最前(已有的从原位置抽出);其余保持用户原顺序;末尾再补默认新模型
            remaining = [m for m in cleaned if m not in APIMODELS_PRIORITY_MODELS]
            current["image_models"] = model_list_from_values(
                [*APIMODELS_PRIORITY_MODELS, *remaining, *APIMODELS_DEFAULT_IMAGE_MODELS]
            )
    # 把三个内置平台按 [apimodels, modelscope, runninghub] 摆在最前
    canonical_order = ["apimodels", "modelscope", "runninghub"]
    builtin = sorted(
        [item for item in merged if item.get("id") in canonical_order],
        key=lambda x: canonical_order.index(x.get("id")),
    )
    others = [item for item in merged if item.get("id") not in canonical_order]
...
class OnlineImageRequest(BaseModel):
    # 允许空 prompt(部分 ai-app 纯图生图无需 prompt);非 ai-app 模型在 build_online_image_result 里二次校验
...
def extract_image(data):
    ...
    # apimodels: {"data":{"state":"completed","resultUrls":["https://..."]}}
    if isinstance(data.get("data"), dict):
        nested_urls = data["data"].get("resultUrls") or data["data"].get("result_urls")
        if isinstance(nested_urls, list) and nested_urls:
            return {"type": "url", "value": nested_urls[0]}
    root_urls = data.get("resultUrls") or data.get("result_urls")
    if isinstance(root_urls, list) and root_urls:
        return {"type": "url", "value": root_urls[0]}
...
def is_apimodels_provider(provider):
    base_url = str((provider or {}).get("base_url") or "").lower()
    return provider_protocol(provider) == "apimodels" or "apimodels.app" in base_url
...
async def wait_for_image_task(client, task_id, provider=None):
    ...
    is_apimodels = is_apimodels_provider(provider)
    is_apimart = is_apimart_provider(provider) and not is_apimodels
    request_params = None
    if is_apimodels:
        # apimodels 复用生成接口轮询：GET /images/generations?task_id=xxx
        task_url = provider_endpoint_url(provider, "image_generation_endpoint", "/v1/images/generations")
    ...
    timeout = APIMART_IMAGE_TASK_TIMEOUT if (is_apimart or is_apimodels) else IMAGE_TASK_TIMEOUT
    interval = APIMART_IMAGE_POLL_INTERVAL if (is_apimart or is_apimodels) else IMAGE_POLL_INTERVAL
    ...
    while time.monotonic() < deadline:
        ...
        if is_apimodels:
            ...
            if state in {"completed", "complete", "done", "success", "succeed", "succeeded", "finished"}:
                return last_payload
            if state in {"failed", "fail", "error", "errored", "canceled", "cancelled", "timeout", "rejected", "expired"}:
                reason = (
                    task_data.get("failMsg") or task_data.get("fail_msg")
                    or task_data.get("message") or last_payload.get("msg")
                    or last_payload.get("message") or "生图任务失败"
                )
...
async def save_ai_image_to_output(image_data, prefix="online_", category="output"):
    ...
    if value.startswith("/output/") or value.startswith("/assets/"):
        return value
    # 远程 URL:尝试下载到本地以便长期保留;若本机到上游网络抖动,退回原远程 URL 让前端直接加载。
    ...
    for attempt in range(3):
        try:
            ...
            if attempt < 2:
                ...
                print(f"[save_ai_image] download attempt {attempt+1} failed ({type(e).__name__}): {e!r}; retry in {backoff}s")
                await asyncio.sleep(backoff)
    print(f"[save_ai_image] 3 次下载均失败,退回远程 URL 让前端直接加载: {value[:120]} (last error: {type(last_exc).__name__}: {last_exc!r})")
    return value
...
APIMODELS_LEGACY_MODEL_MAP = {
    # 我们早期猜的简短 ID → apimodels 真实支持的 ID
    "grok-image": "grok-image/grok-4.2-image",
    "grok-imagine": "grok-imagine/grok-imagine-image",
    "grok-imagine-pro": "grok-imagine-pro/grok-imagine-image-pro",
    "gemini-image": "gemini-3-pro-image-lite",
    "doubao-seedream": "doubao-seedream-4-5-251128",
}

def normalize_apimodels_model(model: str) -> str:
    """把画布历史保存的旧 ID 映射到真实 ID。其他原样返回。"""
    key = (model or "").strip()
    return APIMODELS_LEGACY_MODEL_MAP.get(key, key)

def apimodels_model_kind(model: str) -> str:
    """根据 apimodels 模型 ID 判定模型族,决定 body 形状。"""
    m = (model or "").lower().strip()
    if "grok-imagine" in m and "pro" not in m:
        return "grok-imagine"
    if "grok" in m:
        return "grok"
    if "gemini" in m or "nanobanana" in m:
        return "gemini"
    if "doubao-seedream-5" in m:
        return "doubao-5"
    if "doubao-seedream" in m:
        return "doubao-4"
    return "gpt"  # gpt-image-2 family 默认

def normalize_grok_image_size(size: str) -> str:
    """grok-image / grok-imagine-pro 只接受 1024x1024 / 1536x1024 / 1024x1536 / auto。"""
    width, height = parse_size_pair(size)
    if not width or not height:
        return "1024x1024"
...
def normalize_doubao5_size(size: str) -> str:
    """Doubao Seedream 5.0 Lite 只接受 2K 或 4K(默认 2K)。"""
    s = str(size or "").lower()
    width, height = parse_size_pair(size)
...
def build_apimodels_body(model: str, prompt: str, size: str, image_refs):
    """按模型族构造 apimodels POST body(统一异步 /v1/images/generations)。"""
    kind = apimodels_model_kind(model)
    body = {"model": model, "prompt": prompt}
    aspect_ratio, resolution = apimart_size_resolution(size)
    if kind == "grok-imagine":
        body["aspect_ratio"] = aspect_ratio
    elif kind == "grok":
        body["size"] = normalize_grok_image_size(size)
    elif kind == "gemini":
        body["aspect_ratio"] = aspect_ratio
        body["resolution"] = resolution
    elif kind == "doubao-5":
        body["size"] = normalize_doubao5_size(size)
    elif kind == "doubao-4":
        body["aspect_ratio"] = aspect_ratio
        body["resolution"] = resolution
    else:  # gpt
        body["aspect_ratio"] = aspect_ratio
    if image_refs:
        urls = [reference_to_data_url(ref, max_size=1536) for ref in image_refs[:10]]
...
RUNNINGHUB_WEBAPP_SCHEMA_CACHE: Dict[str, list] = {}
RUNNINGHUB_WEBAPP_SCHEMA_CACHE_LOCK = Lock()

async def fetch_runninghub_aiapp_schema(client, webapp_id: str):
    """用 RUNNINGHUB_WEBAPP_COOKIE 调 /api/aiApp/detail 拉 ai-app 输入 schema。
    返回 schema 里的 nodeInfoList(模板,包含 nodeId/fieldName/fieldType/默认值等),或 None。"""
    cookie = os.getenv("RUNNINGHUB_WEBAPP_COOKIE", "").strip()
    if not cookie:
        return None
    with RUNNINGHUB_WEBAPP_SCHEMA_CACHE_LOCK:
        if webapp_id in RUNNINGHUB_WEBAPP_SCHEMA_CACHE:
            return RUNNINGHUB_WEBAPP_SCHEMA_CACHE[webapp_id]
    headers = {"Cookie": cookie, "Accept": "application/json", "User-Agent": "Mozilla/5.0"}
    for host in ("https://www.runninghub.ai", "https://www.runninghub.cn"):
        try:
            r = await client.get(f"{host}/api/aiApp/detail?aiAppId={webapp_id}", headers=headers, timeout=15)
            if r.status_code != 200:
                continue
            data = r.json()
            if not isinstance(data, dict) or data.get("code") not in (0, 200):
                print(f"[ai-app schema] {host} returned code={data.get('code')} msg={data.get('msg')}")
                continue
            payload = data.get("data") or {}
            # 可能的字段路径:data.nodeInfoList / data.appInputList / data.inputs
            for key in ("nodeInfoList", "appInputList", "inputList", "inputs"):
                lst = payload.get(key)
                if isinstance(lst, list) and lst:
                    with RUNNINGHUB_WEBAPP_SCHEMA_CACHE_LOCK:
                        RUNNINGHUB_WEBAPP_SCHEMA_CACHE[webapp_id] = lst
                    return lst
            # 也试一下 data.workflowInput / data.config.nodeInfoList
            wf = payload.get("workflow") or payload.get("workflowInput") or payload.get("config") or {}
                ...
                for key in ("nodeInfoList", "appInputList", "inputList", "inputs"):
                    ...
                    if isinstance(lst, list) and lst:
                        with RUNNINGHUB_WEBAPP_SCHEMA_CACHE_LOCK:
                            RUNNINGHUB_WEBAPP_SCHEMA_CACHE[webapp_id] = lst
                        return lst
            print(f"[ai-app schema] {host} response 没找到 nodeInfoList,顶层 keys={list(payload.keys())[:20]}")
        except Exception as e:
            print(f"[ai-app schema] fetch from {host} failed: {type(e).__name__}: {e}")
    return None

def auto_build_aiapp_node_info(schema_list, prompt: str, image_urls):
    """根据 schema 自动把 prompt / image_urls 注入对应的 nodeId+fieldName,其余字段沿用默认值。"""
    out = []
    prompt_used = False
    image_idx = 0
    for item in schema_list or []:
        ...
        if not node_id or not field_name:
            continue
        entry = {"nodeId": str(node_id), "fieldName": str(field_name)}
        fname = str(field_name).lower()
        desc = (str(item.get("description") or "")).lower()
        is_prompt_like = any(k in fname or k in desc for k in ("prompt", "text", "正向", "提示"))
        is_image_like = any(k in fname or k in desc for k in ("image", "img", "图片", "参考图", "picture"))
        if (not prompt_used) and prompt and is_prompt_like:
            entry["fieldValue"] = prompt
            prompt_used = True
        elif is_image_like and image_idx < len(image_urls):
            entry["fieldValue"] = image_urls[image_idx]
            image_idx += 1
        else:
            default_val = item.get("fieldValue") if "fieldValue" in item else item.get("defaultValue")
            if default_val is None or default_val == "":
                # 必填但我们没值 → 跳过,让 RunningHub 用 app 自带默认
                continue
            entry["fieldValue"] = default_val
        out.append(entry)
    return out

RUNNINGHUB_WEBAPP_LOCK = Lock()

def load_runninghub_webapps():
    if not os.path.exists(RUNNINGHUB_WEBAPPS_FILE):
        return []
    try:
        with open(RUNNINGHUB_WEBAPPS_FILE, "r", encoding="utf-8") as f:
            data = json.load(f)
        if isinstance(data, list):
            return [w for w in data if isinstance(w, dict) and w.get("webapp_id")]
    except Exception as e:
        print(f"[runninghub_webapps] load failed: {e}")
    return []

def save_runninghub_webapps(webapps):
    os.makedirs(DATA_DIR, exist_ok=True)
    with RUNNINGHUB_WEBAPP_LOCK:
        with open(RUNNINGHUB_WEBAPPS_FILE, "w", encoding="utf-8") as f:
            json.dump(webapps, f, ensure_ascii=False, indent=2)

def find_runninghub_webapp(webapp_id: str):
    target = str(webapp_id or "").strip()
    if not target:
        return None
    for w in load_runninghub_webapps():
        if str(w.get("webapp_id")) == target:
            return w
    return None

# 字段关键词词表(可扩展)
RH_PROMPT_KEYS = {"prompt", "text", "positive_prompt", "positive", "input_text", "description"}
RH_IMAGE_KEYS  = {"image", "img", "input_image", "ref_image", "reference"}
RH_AUDIO_KEYS  = {"audio", "voice", "music"}
RH_VIDEO_KEYS  = {"file", "video", "movie", "clip"}

def classify_runninghub_node_field(item):
    """根据 fieldName + description 判断字段类型。返回 'prompt'|'image'|'audio'|'video'|'param'。"""
    fname = (item.get("fieldName") or "").lower().strip()
    desc = (item.get("description") or "").lower().strip()
    if fname in RH_PROMPT_KEYS or any(k in desc for k in ("提示词", "prompt", "正向")):
        return "prompt"
    if fname in RH_IMAGE_KEYS or any(k in desc for k in ("image", "图片", "参考图", "picture")):
        return "image"
    if fname in RH_AUDIO_KEYS or "audio" in desc or "音频" in desc:
        return "audio"
    if fname in RH_VIDEO_KEYS or "video" in desc or "视频" in desc:
        return "video"
    return "param"

def parse_runninghub_curl_doc(text: str):
    """从 RH 网站复制的 cURL/文档文本里抽出 webapp_id + 整段 nodeInfoList 模板 + 字段分类。"""
    if not text:
        return None
    url_match = re.search(r"/openapi/v2/run/ai-app/(\d+)", text)
    if not url_match:
        return {"error": "未找到 /openapi/v2/run/ai-app/{id} 的接口路径,请确认粘贴的是 RunningHub 的 ai-app cURL 文档"}
    webapp_id = url_match.group(1)
    block = re.search(r"--data-raw\s+'(\{[\s\S]*?\})'", text[url_match.end():])
    if not block:
        block = re.search(r"--data\s+'(\{[\s\S]*?\})'", text[url_match.end():])
    if not block:
        return {"error": "未在 cURL 里找到 --data-raw '...' JSON 体,请粘贴完整的 cURL 命令"}
    try:
        body = json.loads(block.group(1))
    except Exception as e:
        return {"error": f"nodeInfoList JSON 解析失败:{e}"}
    node_list = body.get("nodeInfoList") or []
    if not isinstance(node_list, list) or not node_list:
        return {"error": "cURL 里 nodeInfoList 为空,无法识别输入字段"}
    classified = {"prompt": [], "image": [], "audio": [], "video": [], "param": []}
    template = []
    for item in node_list:
        if not isinstance(item, dict):
            continue
        nid = str(item.get("nodeId", "")).strip()
        fname = str(item.get("fieldName", "")).strip()
        if not nid or not fname:
            continue
        category = classify_runninghub_node_field(item)
        entry = {
            "nodeId": nid,
            "fieldName": fname,
            "description": str(item.get("description") or ""),
            "category": category,
            "default_value": item.get("fieldValue", ""),
        }
        template.append(entry)
        classified[category].append({"nodeId": nid, "fieldName": fname, "description": entry["description"]})
    return {
        "webapp_id": webapp_id,
        "template": template,
        "classified": classified,
        "instance_type": body.get("instanceType") or "default",
    }

def parse_runninghub_ai_app_model(model: str):
    """
    解析 ai-app 模型名编码。返回 (webappId, prompt_spec, image_spec) 或 None。
    格式:
        ai-app/{webappId}                             仅触发,不注入
        ai-app/{webappId}/p{nodeId}                   prompt → node X, fieldName='prompt'
        ai-app/{webappId}/p{nodeId}.{fieldName}       prompt → node X, 自定义 fieldName
        ai-app/{webappId}/p{nodeId}/i{nodeId}         + 参考图 → node Y, fieldName='image'
        ai-app/{webappId}/p{nodeId}/i{nodeId}.{name}  + 参考图自定义 fieldName(如 file/audio)
    """
    parts = (model or "").strip().strip("/").split("/")
    if len(parts) < 2 or parts[0] != "ai-app":
        return None
    webapp_id = parts[1].strip()
    if not webapp_id:
        return None
    prompt_spec = None
    image_spec = None
    for tok in parts[2:]:
        ...
        if "." in rest:
            nid, fname = rest.split(".", 1)
        else:
            nid, fname = rest, ("prompt" if prefix == "p" else "image")
        if prefix == "p":
            prompt_spec = (nid, fname)
        elif prefix == "i":
            image_spec = (nid, fname)
    return webapp_id, prompt_spec, image_spec

async def generate_runninghub_ai_app(prompt, parsed, reference_images, provider):
    webapp_id, prompt_spec, image_spec = parsed
    base = (provider.get("base_url") or RUNNINGHUB_DEFAULT_BASE_URL).rstrip("/")
    url = f"{base}/openapi/v2/run/ai-app/{webapp_id}"
    async with httpx.AsyncClient(timeout=httpx.Timeout(connect=20.0, read=1800.0, write=180.0, pool=20.0)) as client:
        # 先把所有参考图上传,得到 RH download_url 列表
        uploaded_image_urls = []
        for ref in (reference_images or [])[:10]:
            up = await runninghub_upload_reference(client, provider, ref)
            if up:
                uploaded_image_urls.append(up)
        node_info_list = []
        # 优先级 1:手工 /p /i 映射
        if prompt_spec or image_spec:
            if prompt_spec and prompt:
                nid, fname = prompt_spec
                node_info_list.append({"nodeId": nid, "fieldName": fname, "fieldValue": prompt})
            if image_spec and uploaded_image_urls:
                nid, fname = image_spec
                node_info_list.append({"nodeId": nid, "fieldName": fname, "fieldValue": uploaded_image_urls[0]})
        else:
            # 优先级 2:已保存的 webapp 模板(用户粘 cURL 添加过)
            saved = find_runninghub_webapp(webapp_id)
            if saved and isinstance(saved.get("template"), list):
                image_iter = iter(uploaded_image_urls)
                prompt_used = False
                for tpl in saved["template"]:
                    if not isinstance(tpl, dict): continue
                    nid = str(tpl.get("nodeId", "")).strip()
                    fname = str(tpl.get("fieldName", "")).strip()
                    if not nid or not fname: continue
                    category = tpl.get("category") or "param"
                    entry = {"nodeId": nid, "fieldName": fname}
                    if category == "prompt" and not prompt_used and prompt:
                        entry["fieldValue"] = prompt
                        prompt_used = True
                    elif category == "image":
                        try:
                            entry["fieldValue"] = next(image_iter)
                        except StopIteration:
                            # 没足够图,跳过这个 image 字段(让 RH 用默认或报错)
                            default_val = tpl.get("default_value", "")
                            if default_val == "": continue
                            entry["fieldValue"] = default_val
                    else:
                        default_val = tpl.get("default_value", "")
                        if default_val == "": continue
                ...
                if not node_info_list:
                    raise HTTPException(
                        status_code=400,
                        detail=f"ai-app {webapp_id} 的模板未匹配到任何可填字段,请检查保存的 cURL 模板。"
                    )
            else:
                # 优先级 3:Cookie 模式(可选保留)
                schema = await fetch_runninghub_aiapp_schema(client, webapp_id)
                if schema:
                    node_info_list = auto_build_aiapp_node_info(schema, prompt, uploaded_image_urls)
                    if not node_info_list:
                        raise HTTPException(status_code=400, detail=f"ai-app {webapp_id} 拉到 schema 但没匹配上 prompt/image 字段。")
                else:
                    raise HTTPException(
                        status_code=400,
                        detail=f"ai-app {webapp_id} 没有保存的模板。请到 API 设置 → RunningHub → '+ 添加 AI App' 粘贴这个 app 的 cURL 调用示例(在 RunningHub 网站 ai-app 详情页可复制)。"
                    )
        body = {"nodeInfoList": node_info_list, "instanceType": "default"}
        response = await client.post(url, headers=runninghub_api_headers(provider), json=body)
        response.raise_for_status()
        raw = response.json()
        # ai-app 偶尔在初次响应就返回 SUCCESS(同步极快的应用),否则拿 taskId 轮询
        try:
            return runninghub_extract_image(raw), raw
        except HTTPException:
            pass
        # 失败直接抛
        status_raw = str((raw.get("status") if isinstance(raw, dict) else "") or "").upper()
        if status_raw == "FAILED":
            err = (raw.get("errorMessage") or raw.get("errorCode") or raw)
            raise HTTPException(status_code=502, detail=f"RunningHub AI App 任务失败：{err}")
        task_id = runninghub_extract_task_id(raw)
        if not task_id:
            raise HTTPException(status_code=502, detail=f"RunningHub ai-app 未返回 taskId：{raw}")
        result = await wait_for_runninghub_image_task(client, provider, task_id)
        return runninghub_extract_image(result), result

def runninghub_ensure_min_size(width: int, height: int):
    """RunningHub seedream-v5-lite 要求最小边 >= 1600 且总像素 >= 3,686,400(约 1920²)。
    画布默认尺寸常常达不到,这里按比例自动放大,保留 aspect ratio,并对齐到 16 的倍数。"""
    MIN_EDGE = 1600
...
async def generate_runninghub_provider_image(prompt, size, model, reference_images=None, provider=None):
    parsed_app = parse_runninghub_ai_app_model(model)
    if parsed_app is not None:
        return await generate_runninghub_ai_app(prompt, parsed_app, reference_images, provider)
    endpoint = runninghub_task_endpoint(provider, model)
    width, height = parse_size_pair(size)
    width, height = runninghub_ensure_min_size(width, height)
...
async def generate_ai_image(prompt, size, quality, model, reference_images=None, provider_id="comfly"):
    ...
    if quality not in {"low", "medium", "high"}:
        quality = ""
    if is_gpt_image_2_model(model) and not is_apimart and not is_apimodels:
    ...
    request_timeout = httpx.Timeout(connect=20.0, read=1800.0, write=120.0, pool=20.0) if (is_gpt2 or is_apimart or is_apimodels) else AI_REQUEST_TIMEOUT
    async with httpx.AsyncClient(timeout=request_timeout) as client:
        ...
        if is_apimodels:
            model = normalize_apimodels_model(model)
            body = build_apimodels_body(model, prompt, size, image_refs)
            target_url = gen_url
            # apimodels.app 偶发 TLS/连接抖动:连接级错误重试 2 次(指数回退)。
            # 只对 ConnectError/ReadError/RemoteProtocolError 等传输错误重试;HTTPStatusError 让外层处理。
            last_exc = None
            for attempt in range(3):
                try:
                    response = await client.post(target_url, headers=api_headers(provider=provider), json=body)
                    last_exc = None
                    break
                except (httpx.ConnectError, httpx.ReadError, httpx.WriteError, httpx.RemoteProtocolError, httpx.ConnectTimeout) as exc:
                    last_exc = exc
                    if attempt == 2:
                        break
                    backoff = 1.0 * (2 ** attempt)
                    print(f"[apimodels] POST attempt {attempt+1} failed ({type(exc).__name__}): {exc!r}; retry in {backoff}s")
                    await asyncio.sleep(backoff)
...
# --- RunningHub AI App (webapp) 管理 ---

class RhCurlParseRequest(BaseModel):
    text: str = ""

class RhWebappSavePayload(BaseModel):
    webapp_id: str
    name: str = ""
    template: List[Dict[str, Any]] = []
    instance_type: str = "default"

@app.post("/api/runninghub/webapps/parse")
async def runninghub_parse_curl(payload: RhCurlParseRequest):
    result = parse_runninghub_curl_doc(payload.text or "")
    if not result:
        raise HTTPException(status_code=400, detail="未识别到内容")
    if isinstance(result, dict) and result.get("error"):
        raise HTTPException(status_code=400, detail=result["error"])
    return result

@app.get("/api/runninghub/webapps")
async def runninghub_list_webapps():
    return {"webapps": load_runninghub_webapps()}

@app.post("/api/runninghub/webapps")
async def runninghub_save_webapp(payload: RhWebappSavePayload):
    webapp_id = (payload.webapp_id or "").strip()
    if not re.fullmatch(r"\d+", webapp_id):
        raise HTTPException(status_code=400, detail="webapp_id 必须是纯数字")
    if not payload.template:
        raise HTTPException(status_code=400, detail="template 不能为空")
    # 清理 template:只留 nodeId/fieldName/description/category/default_value
    cleaned = []
    for tpl in payload.template:
        if not isinstance(tpl, dict): continue
        nid = str(tpl.get("nodeId","")).strip()
        fname = str(tpl.get("fieldName","")).strip()
        if not nid or not fname: continue
        cat = tpl.get("category") or "param"
        if cat not in {"prompt","image","audio","video","param"}: cat = "param"
        cleaned.append({
            "nodeId": nid,
            "fieldName": fname,
            "description": str(tpl.get("description") or "")[:200],
            "category": cat,
            "default_value": tpl.get("default_value", ""),
        })
    if not cleaned:
        raise HTTPException(status_code=400, detail="template 清理后为空")
    name = re.sub(r"\s+", " ", str(payload.name or "").strip())[:60] or f"AI App {webapp_id}"
    instance = (payload.instance_type or "default").strip()
    if instance not in {"default", "plus"}: instance = "default"
    webapps = load_runninghub_webapps()
    # upsert by webapp_id
    webapps = [w for w in webapps if str(w.get("webapp_id")) != webapp_id]
    record = {
        "webapp_id": webapp_id,
        "name": name,
        "template": cleaned,
    ...
    webapps.append(record)
    save_runninghub_webapps(webapps)
    # 同步注入到 RunningHub provider 的 image_models(model 名 = ai-app/{webapp_id})
    providers = load_api_providers()
    for p in providers:
        if p.get("id") == "runninghub":
            model_name = f"ai-app/{webapp_id}"
            if model_name not in p.get("image_models", []):
                p["image_models"] = [*(p.get("image_models") or []), model_name]
            break
    save_api_providers(providers)
    return {"webapp": record}

@app.delete("/api/runninghub/webapps/{webapp_id}")
async def runninghub_delete_webapp(webapp_id: str):
    webapps = load_runninghub_webapps()
    new_list = [w for w in webapps if str(w.get("webapp_id")) != webapp_id]
    if len(new_list) == len(webapps):
        raise HTTPException(status_code=404, detail="webapp_id 不存在")
    save_runninghub_webapps(new_list)
    # 也从 RunningHub provider 的 image_models 移除
    providers = load_api_providers()
    for p in providers:
        if p.get("id") == "runninghub":
            p["image_models"] = [m for m in (p.get("image_models") or []) if m != f"ai-app/{webapp_id}"]
            break
    save_api_providers(providers)
    return {"deleted": webapp_id}

async def build_online_image_result(payload: OnlineImageRequest):
    provider = get_api_provider(payload.provider_id)
    default_model = (provider.get("image_models") or [IMAGE_MODEL])[0]
    model = selected_model(payload.model, default_model)
    # ai-app 模型可以无 prompt(纯图生图);其它模型仍要求非空
    is_rh_ai_app = is_runninghub_provider(provider) and parse_runninghub_ai_app_model(model) is not None
    if not is_rh_ai_app and not payload.prompt.strip():
    ...
    try:
        ...
        print(f"[generate_ai_image] httpx.{exc_class}: {exc_msg}")
        raise HTTPException(status_code=502, detail=f"请求上游生图接口失败 ({exc_class}): {exc_msg}") from exc
